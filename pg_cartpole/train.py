...
if __name__ == "__main__":
    ...
    for episode in trange(config.episode_count):
        ...
        for t, (obs, action) in enumerate(zip(episode_states, episode_actions)):
            G_t = torch.tensor([returns[t]], dtype=torch.float32)

            logits = policy_net(state_to_tensor(obs))
            log_probs = torch.log_softmax(logits, dim=1)
            log_prob = log_probs[0, action]

            baseline = value_net(state_to_tensor(obs))
            advantage = (G_t - baseline).detach()

            total_policy_loss += -advantage * log_prob
            total_value_loss += (G_t - baseline) ** 2
    ...
    torch.save(policy_net.state_dict(), "models_cartpole/cartpole_pg_policy.pth")
    print("Model saved to models_cartpole/cartpole_pg_policy.pth")
    torch.save(value_net.state_dict(), "models_cartpole/cartpole_pg_value.pth")
    print("Model saved to models_cartpole/cartpole_pg_value.pth")
